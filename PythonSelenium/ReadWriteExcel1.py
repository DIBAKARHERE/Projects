...
for i in range(1, sheet.max_row+1):   # loop run between row 1 to 7
    if sheet.cell(row=i, column=3).value == "Testcase2":   # row 1-7, column 3
        for j in range(1, sheet.max_column+1):
            print(sheet.cell(row=i, column=j).value) # print entire row